---
title: "Efficient leave-one-out cross-validation for Bayesian non-factorizable normal and student-t models"
...
abstract: |
  ...
  A lot of such non-factorizable models fall into the category of multivariate
  normal models, which can be generalized to multivariate student-t models for
  increased robustness against outliers. We derive how to efficiently compute
  and validate both exact and approximate LOO-CV for Bayesian non-factorizable
  models with a multivariate normal or student-t likelihood. In a case study, we
  apply this method to lagged simultaneously autoregressive (SAR)
  models.\linebreak\linebreak Keywords: cross-validation, Pareto-smoothed
  importance-sampling, non-factorizable models, Bayesian inference, SAR models
lang: en-US
class: man
figsintext: true
numbersections: true
...
In this paper, we focus on the expected log predictive density (ELPD) as measure
of predictive accuracy. The ELPD takes the whole predictive distribution into
account and is less focussed on the bulk of the distribution as are other common
measures such as root mean squared error (RMSE) or mean absolute error [MAE; see
...
joint likelihood of the model over all observations has to be factorizable. That
is, the observations have to be pairwise conditionally independent given the
model parameters. However, many important models do not have this property.
...
that the likelihood does not factorize. This is typically due to the fact that
observations depend on other observations from different time periods or
different spatial units in addition to the dependence on the model parameters.

It is well known that (multivariate) normal models are easily influenced by
outliers due to the light tails of the (multivariate) normal distribution [e.g.,
@schwager1982]. Hence, generalizations of the multivariate normal distribution
have been suggested that are more robust against outliers, most notably the
multivariate student-t distribution [e.g., @zellner1976]. This distribution has
an additional positive *degrees of freedom* parameter usually denoted as $\nu$,
which controls the tails of the distribution. If $\nu$ small, the tails are very
fat making outliers less surprising under the implied distribution and thus less
influencial for the resulting parameter estimates. If $\nu$ is large, the
...
student-t models, the fatness of the tails is flexibly adjusted based on pattern
of the observed response values.
...
covariance matrix, regardless of whether or not the likelihood factorizes. We
also provide equations for computing exact LOO-CV for these models, which can be
used to validate the approximation and to handle problematic observations. 
Throughout, a Bayesian model specification
...
# Pointwise log-likelihood for non-factorizable models {#pwnf}
...
be avoided). The pointwise log-likelihood $p(y_i\,|\, y_{-i}, \theta)$ is
straightforward to compute for \emph{factorizable} models in which response
values are conditionally independent given $\theta$, that is, if 
$p(y_i\,|\,y_{-i}, \theta) = p(y_i\,|\, \theta)$. The overall likelihood can then be
...
\end{equation}
<!-- -->
When $p(y \,|\, \theta)$ factorizes this way, the conditional
pointwise log-likelihood can be obtained easily through 
$\log p(y_i \,|\, \theta)$ for each $i$ with computational cost $O(n)$. 

The situation is more
complicated for \emph{non-factorizable} models in which response values are not
conditionally independent. When there is residual dependence even after
accounting for the model parameters, the conditional pointwise log-likelihood
continues to have the general form $\log p(y_i \,|\, y_{-i}, \theta)$. Computing
this pointwise log-likelihood for non-factorizable models is often impossible,
but there is a large class of multivariate normal and student-t models for which
we will provide efficient analytical solutions in this paper.

## Non-factorizable normal models
...
\end{equation}
<!-- -->
with mean vector $\mu$ and covariance matrix $\Sigma$. Usually, $\mu$ and
$\Sigma$ are functions of the model parameters $\theta$, that is, $\mu =
\mu(\theta)$ and $\Sigma = \Sigma(\theta)$. Unless necessary, we omit the
dependence of $\mu$ and $\Sigma$ on $\theta$ in the following for notational
convenience. Using standard multivariate normal theory [e.g., @tong2012], we know
that the conditional distribution of $p(y_i | y_{-i}, \theta)$ of the $i$th
observation is univariate normal with mean
<!-- -->
\begin{equation}
\label{cmean}
...
\end{equation}
<!-- -->
Here, $\Sigma_{-i}$ is the covariance matrix of all but the $i$th observation,
which is obtainable from $\Sigma$ by removing the $i$th row and column,
$\Sigma^{-1}_{-i}$ is the inverse of $\Sigma_{-i}$, and $\mu_{-i}$ is the mean
vector $\mu$ without the $i$th element. Further, $\sigma_{i,-i}$ and
$\sigma_{-i,i}$ are the $i$th row and column vectors of $\Sigma$, respectively,
without the $i$th element and $\sigma_{ii}$ is the $i$th diagnonal element of
$\Sigma$. Equations \eqref{cmean} and \eqref{csd} can be used already to compute
the pointwise log-likelihood values in the normal case as
<!-- -->
\begin{equation}
\label{lp-normal}
  \log p(y_i \,|\, y_{-i},\theta)
...
$\theta_s$ then constitutes the input for the LOO-CV computations.
However, the resulting procedure is quite inefficient.
Computational cost is usually dominated by the computation of
$\Sigma_{-i}^{-1}$, which is in $O(n^k)$. If $\Sigma$ is dense, we have $k = 3$,
and for sparse $\Sigma$ or reduced rank computations, we have $2 < k < 3$. Since
$\Sigma_{-i}^{-1}$ needs to be computed for each $i$, this increases the overall
complexity to $O(N^{k + 1})$. If $\Sigma_{-i}$ additionally depends on the model
parameters $\theta$ in a non-trivial manner, which is the case for most models
of practical relevance, it needs to be inverted for each of the $S$ posterior
draws. Thus, the overall complexity increases further to $O(S N^{k+1})$, which
can be considered quite inefficient. Accordingly, we seek to find
more efficient expressions for $\tilde{\mu}_{i}$ and $\tilde{\sigma}_{i}$
that bring it back into computationally feasible regions.
<!-- -->
\begin{proposition}
...
\end{proposition}
<!-- -->
The proof based on results of
@sundararajan2001 is shown in the Appendix. Contrary to the brute force
computations in \eqref{cmean} and \eqref{csd} where $\Sigma_{-i}$ has to be
inverted separately for each $i$, \eqref{cmean2} and \eqref{csd2} require
inverting the full covariance matrix $\Sigma$ only once and reuse it for each
$i$. Thus, the computational cost reduces to $O(N^k)$ if $\Sigma$ is independent
of $\theta$ and $O(S N^k)$ otherwise. If the model is parameterized
in terms of the covariance matrix $\Sigma = \Sigma(\theta)$, it is not possible
to reduce the complexity further as inverting $\Sigma$ is unavoidable.
However, if the model is parameterized
directly through the inverse of $\Sigma$, that is $\Sigma^{-1} = \Sigma^{-1}(\theta)$,
complexity goes down to $O(S N^2)$. Note that the latter is not possible in 
the brute force approach as both $\Sigma$ and $\Sigma^{-1}$ are required
in this case.
...
## Non-factorizable student-t models
...
\end{equation}
<!-- -->
with degrees of freedom $\nu$, location vector $\mu$
and scale matrix $\Sigma$. Note that, $\mu$ is the mean of $y$ if $\nu > 1$ and
$\frac{\nu}{\nu-2}\Sigma$ is the covariance matrix if $\nu > 2$.
Similar to the multivariate normal case, the conditional distribution
of the $i$th observation given all other observations $p(y_i | y_{-i}, \theta)$
...
\end{proposition}
<!-- -->
The proof based on results of @shah2014 is given in the Appendix.
Note that $\tilde{\mu}_{i}$ is the same as in the normal case and $\tilde{\sigma}_{i}$
...
However, efficiency can again be improved.
\begin{proposition}
...
\end{proposition}
The proof is provided in the Appendix. Computing $\beta_{-i}$ for
a single $i$ is an $O(N^2)$ operation, which needs to be repeated for each
observation. So the cost of computing $\beta_{-i}$ for all observations is
$O(N^3)$. The cost of inverting $\Sigma$ continues to be $O(N^k)$ and so the
overall cost is dominated by $O(N^3)$, or $O(S N^3)$ if $\Sigma$ depends on the
model parameters $\theta$ in a non-trivial manner. That is, in contrast to the
normal case, we cannot reduce the computational costs below $O(S N^3)$ even
if the model was parameterized directly in terms of $\Sigma^{-1} = \Sigma^{-1}(\theta)$ and thus avoided matrix inversion altogether.
However, we are still substantially more efficient than the brute force
approach, which requires $O(S N^{k+1}) > O(SN^3)$ operations.


## Example: Lagged SAR models

It often requires additional work to take a given multivariate or student-t
...
# Approximate LOO-CV for non-factorizable models {#approx-loo-cv}
...
the present data. In particular, since normal models are easily influenced by
outliers, we should be worried about the robustness of the parameter estimates
in the light of the outlying data point. Accordingly, it is sensible to fit a
student-t SAR model as a potentially better predicting and more robust
alternative. We choose an informative ${\rm Gamma}(4, 0.5)$ prior (with mean $8$
and standard deviation $4$) on the degrees of freedom parameter $\nu$ to ensure
rather fat tails of the likelihood and thus greater robustness against outliers.
For all other parameters, we continue to use the weakly-informative default
priors of brms. In Figure \@ref(fig:plots-student) (a), the marginal posterior
distributions of the main model parameters are depicted. Comparing the results
to those shown in Figure \@ref(fig:plots-normal) (a), we see that the
estimates of both the regression parameters and the SAR autocorrelation are
quite similar to the estimates obtained from the normal model.
...
estimation). The results showns in Figure \@ref(fig:plots-student) (c) and (d)
provide further evidence for both, the correctness of our (exact and
approximate) LOO-CV methods for non-factorizable student-t models and for the
goodness of the PSIS-LOO-CV approximation for the present student-t SAR model.
...
similar. Only for the 4th observation, the student-t model fits slightly but
noticably better.
...
In the present paper, we have derived how to perform and validate exact and
approximate leave-one-out cross-validation (LOO-CV) for non-factorizable
multivariate normal and student-t models; models which are highly relevant to
temporal and spatial statistics. The developed LOO-CV approximations make model
fit evaluation and comparison feasible, efficient, and robust. We thus expect them
to fine widespread application. The primary motivation for
this paper is to enable approximate LOO-CV for models that cannot be factorized
at all, but our approach also works for *any* Bayesian model that can be
expressed in terms of a multivariate normal or student-t likelihood. Therefore
it may also be useful for models that are factorizable but for which the
factorized representation is difficult to compute or not available to the
