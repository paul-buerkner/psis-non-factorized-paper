...
*factorized* model. To better illustrate possible structures of the pointwise
...
\begin{minipage}{.48\textwidth}
  ...
  %\captionof{figure}{Another figure}
\end{minipage}
\caption{Directional graphs illustrating common likelihood formulations
...
regression model). (b) Conditionally dependent responses with a Markov property
...
Markov model). (e) Non-factorized model with a joint likelihood over all
responses.}
\label{fig:dags}
\end{figure}
...
the manifest responses $y$ for increased efficiency [e.g., @rasmussen2003].
