...
A standard assumption of any such fast LOO-CV approach using the ELPD is that the
the observation model over all observations has to have a factorized form. That
is, the observation model for each observation is conditionally independent.
However, many important models do not have this property.
Particularly in temporal and spatial statistics it is common to fit models with
multivariate normal or Student-$t$ models that have structured covariance
matrices such that the model does not factorize. This is typically due to
...
do not know of any reformulation that converts the observation model into a factorized form.
...
a non-factorized model instead.
...
pointwise pointwise observation models, we speak of a *factorized*
