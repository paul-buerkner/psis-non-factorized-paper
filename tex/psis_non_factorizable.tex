...
log-likelihood values required for the PSIS-LOO-CV approximation. While the computational cost in factorizable case is $O(n)$, in non-factorizable case the computation cost is dominated by computation of $C^{-1}$. For full-rank dense $C$ the computational cost is $O(n^3)$, but for example in the example below $C$ is product of two sparse matrices and computational cost depends on the number of non-zeros, but is less than $O(n^3)$.

Unfortunately, it often requires additional work to take a given multivariate normal
model and express it in the same form as the zero mean Gaussian process 
in \eqref{gp}, which is required in order to apply the equations for the predictive 
...
are larger when units $i$ and $j$ are closer to each other. In a linear model, the predictor 
...
matrix $C$ to be $\sigma^2  (\widetilde{W}^T \widetilde{W})^{-1}$. Now $C^{-1}=\sigma^{-2}\widetilde{W} \widetilde{W}^T$ and the computational cost is dominated by $\widetilde{W}^{-1} \eta$. In SAR models $\widetilde{W}$ is usually sparse, and if sparse matrix operations are used then the computational cost for $C^{-1}$ is less than $O(n^2)$ and for $\widetilde{W}^{-1}$ computational cost is less than $O(n^3)$ (depending on number of non-zeros and the fill pattern).
...
packages for plotting. The complete R code for this case
