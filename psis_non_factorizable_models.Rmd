...
abstract: |
  Cross-validation can be used to measure a model's predictive accuracy for the purpose of model comparison, averaging, or selection. 
  Standard leave-one-out cross-validation (LOO-CV) requires the likelihood to be factorizable, but some important models in temporal and spatial statistics do not have this property. We derive how to
  compute and validate both exact and approximate LOO-CV for 
  Bayesian non-factorizable models with a multivariate normal likelihood
  and demonstrate with an example of modeling spatially correlated 
  crime data.
...
[@geisser1979; @vehtari2002; @ando2010; @vehtari2012]. In the absence of 
new data with which to evaluate predictive performance, one general approach for evaluating a model's predictive accuracy is cross-validation [@vehtari2002]. When doing cross-validation, the
...
One widely used variant of cross-validation is *leave-one-out cross-validation*
...
practically infeasible as it simply requires too much computation time [@vehtari2017loo].
...
parameters. The purpose of this paper is to generalize exact and approximate
LOO-CV to Bayesian models with non-factorizable multivariate normal likelihoods,
...
Further, we use $y$ to indicate the response vector of all $N$ observations and
$y_{-i}$ to indicate the response vector (of length $N-1$) leaving out the $i$th
value. Model parameters are referred to as $\theta$. Throughout, a Bayesian
model specification and estimation via Markov chain Monte Carlo (MCMC) methods
is assumed.
...
The function $p$ will be either a probability density or mass function, depending on whether the outcome is continuous or
discrete. When $p(y)$ can be factorized in this way, the conditional
...
not usually used for LOO-CV of Gaussian process models because, in most the cases,
Gaussian processes are combined with a factorizable likelihood so that simpler
equations for univariate distributions can be applied. 

What makes Sundararajan and Keerthi's equations helpful in the non-factorizable
case is our observation that their derivations make no use of the special form
of $C$ for Gaussian process models and thus immediately generalize to the case
of an arbitrary invertible covariance matrix $C$. For such models, the LOO
predictive mean and standard deviation can be computed as follows:
...
corresponding to the held out observation. But this does not hold in general for
...
# Case Study: spatially correlated crime data
...
neighborhoods of Columbus, Ohio during the year 1980. The data set was originally
...
spatial dependency via an SAR structure, can be specified with brms as
follows:
...
single observation. For the lagged SAR model, we cannot just ignore the
held-out observation entirely as this will change the prior distribution. In other words, the lagged SAR model does not have the
...
spatial SAR model. Although this paper focuses on 
non-factorizable models (i.e. models that cannot be factorized at all), our approach also works for _any_ Bayesian model that can be expressed in
